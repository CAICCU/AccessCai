...
static function KF2GUIController GetGUIController(PlayerController PC)
{
	...
	foreach PC.ChildActors(class'AccessCai.KF2GUIController', G)
	{
		if (!G.bIsInvalid)
		{
			break;
	...
	if (G == None)
	{
		G = PC.Spawn(class'AccessCai.KF2GUIController', PC);
	}
...
simulated function HandleDrawMenu()
{
	if (HackConsole == None)
	{
		HackConsole = new(ClientViewport)class'AccessCai.KFGUIConsoleHack';
...
simulated final function SetMenuState(bool bActive)
{
	...
	if (bActive)
	{
		if (CustomInput == None)
		{
			CustomInput = new (KFPlayerController(PlayerOwner)) class'AccessCai.KF2GUIInput';
