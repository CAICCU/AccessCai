Class AccessCaiWebApp extends Object implements(IQueryHandler);

var WebAdmin webadmin;
var string AccessURL;
var AccessCai AccessControl;
...
function registerMenuItems(WebAdminMenu menu){
	menu.addMenu(AccessURL, "Access Plus", self, "Modify AccessCai settings.", -25);
}
...
function bool handleQuery(WebAdminQuery q){
	if( LastUserIP!=q.user.getUsername() ){
		...
		bAccConfirmed = false;
		//`Log("Start new AccessCai user"@LastUserIP);
	}
...
final function string GrabGroupOutput( WebAdminQuery q, string VarName ){
	...
	for( i=0; i<j; ++i ){
		...
		if( n==-1 ) return "All";
		if( n==-2 ) return "";
		if( i==0 ) S = AccessControl.AdminData.CG[n];
...
function handleACPlus(WebAdminQuery q)
{
	...
	case 2:
		...
		{
			EditSettingLine = AccessControl.WebAddBan(q.user.getUsername(),q.request.getVariable("ID"), q.request.getVariable("STID"));
		}
		else
		{
		...
		q.response.SendText("<tr><th title=\"Unique ban ID for this ban\">BanID</th><th title=\"The admin that initially banned this player\">Admin</th><th>Name</th><th title=\"UniqueId for this user\">UniqueId</th><th title=\"SteamID for this user\">SteamID</th><th>IP</th><th title=\"Ban reason shown for the player themself\">Reason</th><th title=\"Ban expiry time in hours (minutes)\">Expires</th><th title=\"Custom notes for this ban\">Notes</th></tr>");
		for( i=0; i<AccessControl.BansData.BE.Length; ++i )
		{
			if( EditSettingLine==i )
			{
				...
				q.response.SendText("<td>"$AccessControl.BansData.BE[i].STID$"</td>",false);
			...
			else
			{
				q.response.SendText("<tr><td>"$AccessControl.BansData.BE[i].IX$
									...
									"</td><td>"$AccessControl.BansData.BE[i].STID$
		...
		AddInLineEditbox(q,"BannedSteamID","STID");//
