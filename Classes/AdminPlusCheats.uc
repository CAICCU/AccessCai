...
final function SetCommands( string CmdList, AccessData A ){
	...
	if( SA.Length==0 || (SA.Length==1 && SA[0]=="") ) return;
...
final function ShowCommandList(){
	...
	for( i=0; i<CommandList.Length; ++i ){
		if( bGlobalAdmin || (PrivelegedCommands.Length>i && PrivelegedCommands[i]!=0) ){
			if( Len(S)==0 ) S = string(CommandList[i]);
...
final function Class StrToClass( string S ){
	...
	if( InStr(S,".")==-1 ){
		...
		if( Result==None )
		    Result = Class( DynamicLoadObject( "AccessCai."$S, class'Class', true ) );
	}
...
final function bool SetProp( Actor A, string VarName, string Value ){
	...
	switch( Caps(VarName) ){
		...
		return true;
		case "LADDERSPEED": if( Pawn(A)==None )
		return false;
		...
		return true;
		case "TEAM":
		if( Pawn(A)==None || Pawn(A).Controller==None ) return false;
		if( Value=="0" || Value=="1" ){
		...
		return true;
		case "NUMPLAYERS":
		if( GameInfo(A)==None ) return false;
...
exec function TeleP( string ID ){
	...
	`CheckExecuteCheat();
	if ( !FindPlayersByID(ID,C,N,) ) return;
...
exec function GotoP( string ID ){
	...
	if( Pawn!=None ) Pawn.SetLocation(Pos);
...
exec function GiveWP( string weaponClass ){
	...
	if( Pawn==None || Pawn.InvManager==None ) return;

	Inv = class<Inventory>(DynamicLoadObject("KFGameContent."$weaponClass,Class'Class'));

	if( Inv==None )
	    Inv = class<Inventory>(DynamicLoadObject("AccessCai."$weaponClass,Class'Class'));
...
exec function SetSize( String str ){
	...
	j = InStr(str," ");
	if( i==-1 ) F = 1.F;
	else F = int(Left(str,j));

	if ( !FindPlayersByID(ID,C,N,false) ) return;
...
function class<KFPawn_Monster> LoadMonsterByName(){
	...
	SpawnClass = class<KFPawn_Monster>(DynamicLoadObject("AccessCai.KFPawn_FPVS", class'Class'));
...
exec function MagicSummon( string MClass){
	...
	if (KFPawn_M != None){
		...
		if(SpawnVolume != None) SpawnVolume.SpawnWave(FakeSpawnList, true);
	}
}
...
exec function ListActors( class<Actor> BaseClass, optional float Radii ){
	...
	if( BaseClass==None ) BaseClass = class'Actor';
...
exec function Kill( string ID ){
	...
	if ( !FindPlayersByID(ID,C,N,true) ) return;
...
final function bool ForceRespawnPlayer( Controller NewPlayer, vector StartPos, rotator StartRot ){
	...
	if( NewPlayer.Pawn!=None ) NewPlayer.Pawn.Destroy();
	...
	if (KFPC != none){
		...
		if(LP != None) { 
			...
			if(KFPC.myHUD != None) KFPC.myHUD.NotifyBindPostProcessEffects();
		} 
	}

	KFGameInfo(WorldInfo.Game).SetTeam( NewPlayer, KFGameInfo(WorldInfo.Game).Teams[0] );

	if( KFPC != none ) KFPC.InitGameplayPostProcessFX();
	if( KFPRI!=None ){
		if( KFPRI.Deaths == 0 )
...
exec function ReadyUp(){
	...
	ForEach WorldInfo.AllControllers(class'PlayerController', PC){
		if( PC.bIsPlayer ) PC.PlayerReplicationInfo.bReadyToPlay = true;
	}
}
...
exec function KillZeds(){
	...
	ForEach WorldInfo.AllPawns(class'KFPawn_Monster', AIP){
		if( AIP.Health>0 ) AIP.Died(None, None, AIP.Location);
...
exec function TeleportZeds(){
	...
	ForEach WorldInfo.AllPawns(class'KFPawn_Monster', AIP){
		if( AIP.Health>0 ) AIP.SetLocation( HitLocation );
	}
}
...
exec function WeldDoors(){
	...
	foreach WorldInfo.AllActors( class'KFDoorActor', KFDA ){
		if (KFDA.bIsDoorOpen) KFDA.UseDoor(Pawn);

		if (!KFDA.bIsDoorOpen && !KFDA.bIsDestroyed){
...
exec function setNextwave(int wave){
	...
	if(KFGI!=none) KFGI.WaveNum = wave-1;
